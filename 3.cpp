...
void TRA_switch::recv_handler (packet *p){
    ...
    if (p->type() == "SDN_ctrl_packet") return;
...
void SDN_switch::recv_handler (packet *p){
    ...
    if (p->type() == "TRA_ctrl_packet") { // the switch receives a packet from the controller  action like TRA_switch
        ...
        if(router_table.find(p3->getHeader()->getSrcID()) == router_table.end()){//if the destination is not in router table
            ...
            router_table[p3->getHeader()->getSrcID()].push_back(make_pair(temp, 0.5));//default partion 50%
        }
        else{
    ...
    if (p->type() == "SDN_ctrl_packet") { // the switch receives a packet from the sdn_ntroller
        ...
        temp.mincounter=0;
        temp.next=act;
        if(per==1.0)router_table[mat].pop_back();//update router table
...
int main()
{
    ...
    int node_num,dst_num,links_num,pairs_num,sim_time,sdn_ctrl_time,budget;
    cin>>node_num>>dst_num>>links_num>>pairs_num>>sim_time>>sdn_ctrl_time>>budget;
    struct node_info{int id; int brocatst_time; bool sdn_type; bool dst_tag=false;};//node information
    struct sdn_score{int id; int cost; int score;};//The score of being a sdn node
    struct node_info *node_info_list=new struct node_info[node_num];
    struct sdn_score *score_list=new struct sdn_score[node_num];
    struct pre_dst{int dst_id; int *parent;unsigned int *dis;};//預先處裡每個destination在圖的shortest path tree
    struct pre_dst *pre_dst_list=new pre_dst[dst_num];
    int **G=new int*[node_num];//建圖
    for(int i=0;i<node_num;i++)G[i]=new int[node_num];
    vector<unsigned int> dstlist;//destination list
    for(int c=0,tmp;c<dst_num;c++){//input destination
        cin>>tmp;
        dstlist.push_back(tmp);
        node_info_list[tmp].dst_tag=true;
        pre_dst_list[c].dst_id=tmp;
        pre_dst_list[c].parent=new int[node_num];
        pre_dst_list[c].dis=new unsigned int[node_num];
    }
    for(int i=0,in1,in2,in3;i<node_num;i++){//input node information
        cin>>in1;
        node_info_list[i].id=in1;
        score_list[i].id=in1;
        score_list[i].score=10;
        node_info_list[i].sdn_type=false;
        if(node_info_list[i].dst_tag){
            cin>>in2>>in3;
            score_list[i].cost=in2;
            node_info_list[i].brocatst_time=in3;
            score_list[i].score-=4;//Scoring mechanism
        }
        else{
            cin>>in2;
            score_list[i].cost=in2;
            node_info_list[i].brocatst_time=-1;
        }
    }
    struct link_info{int pointA; int pointB;};
    struct link_info *link_info_list=new struct link_info[links_num];
    for(int i=0,in1,in2,in3;i<links_num;i++){//input link information
        cin>>in1>>in2>>in3;
        link_info_list[i].pointA=in2;
        link_info_list[i].pointB=in3;
        G[in2][in3]=1;
        G[in3][in2]=1;
        score_list[in2].score+=2;//Scoring mechanism
        score_list[in3].score+=2;//Scoring mechanism
    }
    
    struct pair_info{int src; int dst; int flow;};
    struct pair_info *pairlist=new struct pair_info[pairs_num];
    for(int i=0,in1;i<pairs_num;i++){//input pair information
        cin>>in1>>pairlist[i].src>>pairlist[i].dst>>pairlist[i].flow;
        score_list[pairlist[i].src].score-=4;
        for(int j=0;j<links_num;j++){
            if(link_info_list[j].pointA==pairlist[i].src)score_list[link_info_list[j].pointB].score+=2;//Scoring mechanism
            else if(link_info_list[j].pointB==pairlist[i].src)score_list[link_info_list[j].pointA].score+=2;//Scoring mechanism
        }
    }
    for(int i=0;i<dst_num;i++){//dijkstra
        bool visit[node_num];//記錄各個點是否在路徑當中
        for(int count=0;count<node_num;count++){//初始化
            visit[count]=false;
            pre_dst_list[i].dis[count]=BROCAST_ID;
        }
        pre_dst_list[i].dis[pre_dst_list[i].dst_id]=0;
        pre_dst_list[i].parent[pre_dst_list[i].dst_id]=pre_dst_list[i].dst_id;//起始點設定
        for(int C=0;C<node_num;C++){
            int temp_a=-1;
            unsigned int min=BROCAST_ID;
            for(int k=0;k<node_num;k++){
                if(!visit[k] && pre_dst_list[i].dis[k]<min){
                    temp_a=k;
                    min=pre_dst_list[i].dis[k];
                }
            }
            if(temp_a==-1)break;//起始點所連通的路徑都已找完則跳出
            visit[temp_a]=true;
            for(int temp_b=0;temp_b<node_num;temp_b++){//尋求較短度路徑並且加入路徑
                if(G[temp_a][temp_b]!=0 && G[temp_b][temp_a]!=0 && !visit[temp_b] && pre_dst_list[i].dis[temp_a]+G[temp_a][temp_b] < pre_dst_list[i].dis[temp_b]){
                    pre_dst_list[i].dis[temp_b]=pre_dst_list[i].dis[temp_a]+G[temp_a][temp_b];
                    pre_dst_list[i].parent[temp_b]=temp_a;
                }
            }
        }
    }
    for(int i=0;i<dst_num;i++){//Scoring mechanism
        for(int j=0;j<node_num;j++){
            for(int k=0;k<node_num;k++){
                if(pre_dst_list[i].parent[k]==score_list[j].id)score_list[j].score+=2;
            }
        }
    }
    sdn_score temp;
    for(int i=1;i<node_num;i++){//sort the score list
        int t=i;
        for(int j=i-1;j>=0;j--){
            if(score_list[t].score-0.1*(float)score_list[t].cost > score_list[j].score-0.1*(float)score_list[j].cost){
                temp=score_list[j];
                score_list[j]=score_list[t];
                score_list[t]=temp;
                t=j;
            }
        }
    ...
    int costsum=0;
    vector<int> sdn_set;
    for(int i=0;i<node_num;i++){//choose the sdn node
        if(temp_budget>score_list[i].cost){
            sdn_set.push_back(score_list[i].id);
            temp_budget-=score_list[i].cost;
            costsum+=score_list[i].cost;
        }
        else if(temp_budget==score_list[i].cost){
            sdn_set.push_back(score_list[i].id);
            temp_budget-=score_list[i].cost;
            costsum+=score_list[i].cost;
            break;
    ...
    //sdn_set.clear(); //no sdn
    for(int i=0;i<sdn_set.size();i++){//update node information list
        node_info_list[sdn_set[i]].sdn_type=true;
    }
    for (unsigned int id = 0; id < node_num; id ++){//generate node
        if(node_info_list[id].sdn_type){
    ...
    for(int i=0;i<links_num;i++){// set switches' neighbors
        node::id_to_node(link_info_list[i].pointA)->add_phy_neighbor(link_info_list[i].pointB);
        node::id_to_node(link_info_list[i].pointB)->add_phy_neighbor(link_info_list[i].pointA);
    }
    for(int i=0;i<sdn_set.size();i++){//link every SDN_node to the SDN_controller
    ...
    for(int i=0;i<node_num;i++){
        if(node_info_list[i].dst_tag){
            TRA_ctrl_packet_event(node_info_list[i].id,node_info_list[i].brocatst_time);
        }
    }
    // 1st parameter: the source; the destination that want to broadcast a msg with counter 0 (i.e., match ID)
    // 2nd parameter: time (optional)
    ...
    // for (map<unsigned int,bool>::const_iterator it = nblist.begin(); it != nblist.end(); it ++) {
    //     cout << it->first << ", " ;
    for(int i=0;i<sdn_set.size();i++){//setting next node and portion for sdn_switch
        for(int j=0;j<dst_num;j++){
            int dst=dstlist[j];
            vector<int> path;
            int now=0;
            for(int m=0;m<dst_num;m++)if(pre_dst_list[m].dst_id==dst)now=m;
            const map<unsigned int,bool> &nblist=node::id_to_node(sdn_set[i])->getPhyNeighbors();
            for (map<unsigned int,bool>::const_iterator it = nblist.begin(); it != nblist.end(); it ++) {//select next node from sdn node's neighbors
                    int k;
                    for(k=0;k<pairs_num;k++){
                        if(pairlist[k].src!=it->first && pairlist[k].dst==dst)continue;
                        else break;
                    }
                    if(k==pairs_num && pre_dst_list[now].parent[it->first]!=sdn_set[i] && it->first!=pre_dst_list[now].parent[sdn_set[i]] && it->first!=con_id && !node_info_list[it->first].sdn_type && pre_dst_list[now].parent[it->first]!=pre_dst_list[now].parent[sdn_set[i]])
                        path.push_back(it->first);
                }
            if(path.size()==0){//send sdn_ctrl_packet to sdn_controller
                SDN_ctrl_packet_event(con_id, sdn_set[i], dst, pre_dst_list[now].parent[sdn_set[i]], 1.0, sdn_ctrl_time);
            }
            if(path.size()==1){
                SDN_ctrl_packet_event(con_id, sdn_set[i], dst, path[0], 0.5, sdn_ctrl_time);
            }
            if(path.size()==2){
                SDN_ctrl_packet_event(con_id, sdn_set[i], dst, path[0], 0.25, sdn_ctrl_time);
                SDN_ctrl_packet_event(con_id, sdn_set[i], dst, path[1], 0.25, sdn_ctrl_time);
            }
            if(path.size()==3){
                SDN_ctrl_packet_event(con_id, sdn_set[i], dst, path[0], 0.2, sdn_ctrl_time);
                SDN_ctrl_packet_event(con_id, sdn_set[i], dst, path[1], 0.15, sdn_ctrl_time);
                SDN_ctrl_packet_event(con_id, sdn_set[i], dst, path[2], 0.15, sdn_ctrl_time);
            }
            if(path.size()>=4){
                SDN_ctrl_packet_event(con_id, sdn_set[i], dst, path[0], 0.125, sdn_ctrl_time);
                SDN_ctrl_packet_event(con_id, sdn_set[i], dst, path[1], 0.125, sdn_ctrl_time);
                SDN_ctrl_packet_event(con_id, sdn_set[i], dst, path[2], 0.125, sdn_ctrl_time);
                SDN_ctrl_packet_event(con_id, sdn_set[i], dst, path[3], 0.125, sdn_ctrl_time);
            }
        }
    }
