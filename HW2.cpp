...
class TRA_switch: public node {
    ...
    public:
        ...
        void print_table(unsigned int dst){
...
int main()
{
    ...
    // read the input and generate TRA nodes
    int node_num,dst_num,links_num,pairs_num,sim_time;
    cin>>node_num>>dst_num>>links_num>>pairs_num>>sim_time;
    for (unsigned int id = 0; id < node_num; id++){
        node::node_generator::generate("TRA_switch",id);
    }
    // destination input
    vector<int> dstlist;
    for(int c=0,tmp;c<dst_num;c++){
        cin>>tmp;
        dstlist.push_back(tmp);
    }
    struct brocast{unsigned int id; unsigned int ti;};
    struct brocast *b_list=new struct brocast[dst_num];
    for(int c=0;c<dst_num;c++){
        int id,ti;
        cin>>id>>ti;
        b_list[c].id=id;
        b_list[c].ti=ti;
    }
    for(int c=0;c<links_num;c++){// set switches' neighbors
        int pointA,pointB,i;
        cin>>i>>pointA>>pointB;
    ...
    //pair input
    struct pr{int src; int dst; int size; int s_time;};
    struct pr *prlist=new struct pr[pairs_num];
    for(int c=0,i;c<pairs_num;c++){
        cin>>i>>prlist[c].src>>prlist[c].dst>>prlist[c].size>>prlist[c].s_time;
    }
    

    // dst node broadcasts a msg with counter 0 at time broadcast time
    
    for(int i=0;i<dst_num;i++){
        TRA_ctrl_packet_event(b_list[i].id, b_list[i].ti);
    }
    ...
    for(int i=0;i<pairs_num;i++){
        data_packet_event(prlist[i].src, prlist[i].dst, prlist[i].s_time);
    }
    ...
    //outpot each node's router table
